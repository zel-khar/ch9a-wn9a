...
static int	add_splitted_tokens(char *value, t_token **head)
{
	...
	while (value[i])
	{
        ...
        if (!part)
            return (*head = NULL, 0);
        /* restore protected whitespace for this part */
        restore_ws(part);
...
static int	handle_word_token(const char *str, int *i, t_tokbuild *tb)
{
    ...
    /*
     * For normal words: quoted=1 means fully quoted; set to 0 on any
     * unquoted segment so we can enable field splitting.
     * For heredoc delimiter: quoted=1 means the word had any quotes at all.
     */
    quoted = (*(tb->heredoc_delim) ? 0 : 1);
    ...
    {
        /* Restore protected whitespace before emitting */
        restore_ws(value);
