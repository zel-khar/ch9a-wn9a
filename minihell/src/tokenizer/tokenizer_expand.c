...
static char *tok_handle_quotes(const char *str, int *i, t_tokctx *ctx)
{
    ...
    *i = j + 1;
    /* Protect whitespace inside any quotes from later field splitting */
    k = 0;
...
static int process_word_step(const char *s, int *i, t_tokctx *ctx, char **res)
{
    ...
    if (s[*i] == '\'' || s[*i] == '"')
    {
        if (ctx->heredoc && ctx->quoted)
            *(ctx->quoted) = 1; /* any quote marks heredoc as quoted */
        /* Quoted segment: keep fully-quoted status unless we saw unquoted */
    ...
    if (s[*i] == '$' && !ctx->heredoc)
    {
        ...
        if (!part)
            return (-1);
        /* Unquoted parameter expansion: enables field splitting later */
        if (ctx->quoted)
            *(ctx->quoted) = 0;
        return (join_part(res, part));
    }
    *res = tok_str_append_char(*res, s[*i]);
    (*i)++;
    /* Any literal char outside quotes => not fully quoted (non-heredoc) */
    if (!ctx->heredoc && ctx->quoted)
        *(ctx->quoted) = 0;
    if (*res)
        return (0);
